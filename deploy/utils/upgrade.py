...
async def main():
    async with ClientSession(connector=TCPConnector(ssl=False)) as session:
        ...
        builder = builder.allocate_global_address(
            ...
            ret.ManifestV1BuilderNamedAddress('exchange_component')
        )
        ...
        builder = builder.call_method(
            ret.ManifestBuilderAddress.STATIC(ret.Address(exchange_component)),
            'signal_upgrade',
            [ret.ManifestBuilderValue.ADDRESS_VALUE(ret.ManifestBuilderAddress.NAMED(ret.ManifestV1BuilderNamedAddress('exchange_component')))]
        )
